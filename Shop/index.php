<?php
	...
	{
		$img_name = "pictures/".basename($_FILES["picture"]["name"]);
		$is_img_upload = move_uploaded_file($_FILES["picture"]["tmp_name"], $img_name);
		if(!$is_img_upload) 
		{ 
		    if($_FILES["picture"]["error"] == 2)
		    {
		        echo '<p style="color: red;">Error: Размер загружаемого файла превысил значение 100 Kb</p>';
		    }
		    $img_name = "";
		}
		...
		{
		    if(!$is_img_upload)  $img_name = $_POST["picture"];
